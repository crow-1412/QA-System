    ...
    def embed_documents(self, texts: list[str]) -> list[list[float]]:
        ...
        for text in texts:
            if isinstance(text, Document):
                ...
                content = str(text)
            processed_texts.append(content.replace("\n", " "))
            
        try:
            if self.multi_process:
                print("Using multi-process encoding...")
                pool = self.client.start_multi_process_pool()
                embeddings = self.client.encode_multi_process(processed_texts, pool)
...
class BCERetriever:
    def __init__(self, model_path, data):
        ...
        """
        print(f"\nInitializing BCERetriever with model: {model_path}")
        try:
            self.model = SentenceTransformerMy(model_path, device="cuda")
            self.docs = data
            print("\nCreating vector store...")
            self.vector_store = self._create_vector_store(data)
            print("BCERetriever initialization completed")
        except Exception as e:
            print(f"Error initializing BCERetriever: {str(e)}")
            raise
